...
sub emergeApp{
    ...
    my $found_flag = 0;
		while(<CMD>){
				...
				# check if the flag we wanted appeared.
				if(checkAcceptedFlags($_)){
						$found_flag = 1;
				}
...
# returns true if any of the used flags appear in the given string
sub checkAcceptedFlags{
		my($string) = @_;
		for my $pattern (@usedFlags){
						if($string =~ /$pattern/){
								return 1;
						}
		}
